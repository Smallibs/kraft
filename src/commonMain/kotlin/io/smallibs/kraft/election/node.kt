...
data class Elector(override val context: Context) : Node(context) {
    fun becomeFollower(candidate: Identifier) = Follower(context, candidate)
...
data class Candidate(override val context: Context, val followers: List<Identifier> = listOf()) : Node(context) {
    ...
    fun becomeCandidate(follower: Identifier) = Candidate(context, followers + follower)
}

data class Follower(override val context: Context, val leader: Identifier, val extendedTime: Boolean = false) :
    Node(context) {
